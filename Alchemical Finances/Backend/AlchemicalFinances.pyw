...
from Frontend.AlchemicalFinancesUi import Ui_MainWindow
from Backend.Ledger1 import LedgerV1
from Backend.Ledger2 import LedgerV2
from Backend.Summary import LedgerS
from Backend.About import AboutProgram
from Backend.Archive import Archive
from Backend.RequestReports import user_report_request
from Backend.Scrape import update_stock_price
from Backend.NWGDisplay import NetWorthGraph
from Backend.UPK import account_pathway, modify_for_sql, specific_sql_statement, set_networth, obtain_sql_value
...
class AFBackbone(QMainWindow):
    ...
    def __init__(self, user, messageCount):
        ...
        if self.create_profile_db() is True:
            ...
            specific_sql_statement(nw_graph_statement, self.refUserDB)
        ...
        netWorth = set_networth("Account_Summary", self.refUserDB, 0)
        self.ui.labelNW.setText(netWorth[1])
        self.ui.labelTA.setText(netWorth[2])
        self.ui.labelTD.setText(netWorth[3])

        snapshot = set_networth("Account_Summary", self.refUserDB, 1)
    ...
    # --- -- Shut Down App via the X button ------------------------------------------------------------------------------
...
To re-access this tab restart the Application."""
    ...
    def switch_tab(self, parentType):
        ...
        try:
            ...
            if parentType == "Summary":
                ...
                graph = NetWorthGraph(self, self.refUserDB)
    ...
    # --- Adds a data point for the users net worth on log in or logout ------------------------------------------------
    def log_networth(self, data, entryPoint):
        ...
        insert_today_statement = "INSERT INTO NetWorth Values('" + today + "','" + data[0] + "','" + data[1] + "','" + data[2] + "')"
        insert_yesterday_statement = "INSERT INTO NetWorth Values('" + yesterday + "','" + data[0] + "','" + data[1] + "','" + data[2] + "')"
        update_statement = "UPDATE NetWorth SET Gross='" + data[0] + "', Liabilities='" + data[1] + "', Net='" + data[2] + "' WHERE Date='" + today + "'"

        last_data_point_statement = "SELECT Date FROM NetWorth ORDER BY Date DESC LIMIT 1"
        last_data_point = obtain_sql_value(last_data_point_statement, self.refUserDB)

        if last_data_point is None:
            last_data_point = "2000/01/01"  # Just an unlikely date
    ...
    #     self.click += 1
    #     self.ui.labelStaticNW.setText("Hello " + str(self.click))
    #     netWorth = str(set_networth("Account_Summary", self.refUserDB, 0))
